...
1. Run the database image to confirm connectivity. It takes some time to discover all of the necessary `podman run` options.
...
$ sudo podman run -d -v /var/lib/mariadb:/var/lib/mysql:Z -p 8080 -e DBUSER=user -e DBPASS=mypassword -e DBNAME=mydb --name mariadb mariadb
```
...
$ sudo podman run -d -v /var/lib/wp_uploads:/var/www/html/wp-content/uploads:Z --network=container:mariadb -e DB_ENV_DBUSER=user -e DB_ENV_DBPASS=mypassword -e DB_ENV_DBNAME=mydb -e DB_HOST=0.0.0.0 -e DB_PORT=3306 --name wordpress wordpress
```
...
1. Edit `wordpress/Dockerfile` and add the following instruction near the bottom of the file above the CMD line.
        ...
        LABEL run podman run -d -v /var/lib/wp_uploads:/var/www/html/wp-content/uploads:Z --network=container:mariadb -e DB_ENV_DBUSER=user -e DB_ENV_DBPASS=mypassword -e DB_ENV_DBNAME=mydb -e DB_HOST=0.0.0.0 -e DB_PORT=3306 --name wordpress wordpress

1. Rebuild the Wordpress image. The image cache will be used so only the changes will need to be built.

        $ sudo podman build -t wordpress wordpress/

1. Re-run the Wordpress image using the `atomic` CLI. We don't need to use a complicated, error-prone `podman run` string. Test using the methods from the earlier step. In addition, we are going to launch the image using the `atomic` command. Before we can do that, we'll install it as noted below.

        $ sudo podman stop wordpress
...
```bash
$ sudo podman ps
$ sudo podman stop mariadb wordpress
```

After iterating through running podman images you will likely end up with many stopped containers. List them.
...
```bash
$ sudo podman rm $(sudo podman ps -qa)
```
